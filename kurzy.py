...
def draw_current_data(data, window):
    ...
    for entry in data:
        if entry['country'] in config['config']['countries']:
            output = entry['country'] + ' (' + entry['shortName'] + ') ' + str(
                entry['currBuy']) + ' ' + str(entry['currSell'])
...
def draw_menu_select(menu_window, item, direction):
    ...
    menu_window.addstr(item, 0, config['config']['countries'][
                       item], curses.color_pair(8) | curses.A_BOLD)
...
if __name__ == '__main__':
    ...
    request = requests.get(
        'https://api.csas.cz/sandbox/webapi/api/v1/exchangerates', headers=headers)
    ...
    data_window = curses.newwin(
        rows, columns - (max_width + 1), 0, max_width + 1)
    ...
    while True:
        ...
        if c == curses.KEY_UP:
            ...
            draw_country_data(data_window, json_data, config[
                              'config']['countries'][current_item])
